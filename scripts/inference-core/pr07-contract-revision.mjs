...
if (
  ...
  throw new Error(
    "Pass exactly --print-operation-policy or --write for the PR-07 contract.",
  )
}
...
const baseEvidenceErrors = verifyPr07BaseEvidence(repositoryRoot)
if (baseEvidenceErrors.length > 0) {
  throw new Error(
    `PR-07 retained prior evidence changed: ${baseEvidenceErrors.join("; ")}`,
  )
}

const baseAllowlist = readJsonFromCommit(pr07ContractBase, allowlistPath)
const baseRoutes = readJsonFromCommit(pr07ContractBase, routeBaselinePath)
...
const boundaryErrors = verifyPr07OperationBoundary(operationPolicy)
if (boundaryErrors.length > 0) {
  throw new Error(
    `PR-07 candidate leaves its approved package boundary: ${boundaryErrors.join("; ")}`,
  )
}
const candidateErrors = verifyPr07CandidateContract({
  root: repositoryRoot,
  baseAllowlist,
  currentAllowlist,
  baseRoutes,
...
if (candidateErrors.length > 0) {
  throw new Error(
    `PR-07 candidate violates the reviewed contract: ${candidateErrors.join("; ")}`,
  )
}
...
const decisionErrors = verifyPr07DecisionDocument(decision, {
  requireReady: true,
})
if (decisionErrors.length > 0) {
  throw new Error(
    `PR-07 decision evidence is not ready: ${decisionErrors.join("; ")}`,
  )
}
if (
  ...
  throw new Error(
    "PR-07 reviewed operation policy differs from staged candidate.",
  )
}

const evidenceFiles = pr07RevisionEvidencePaths.map((path) => ({
  path,
...
const revision = buildContractRevisionDocument({
  ...
  baseTree: gitOutput(["rev-parse", `${pr07ContractBase}^{tree}`]),
  baseAllowlist,
  currentAllowlist,
  baseRoutes,
  currentRoutes: currentRoutesBeforeRevision,
...
writeJsonTransaction([
  { path: pr07ContractRevisionPath, content: revisionContent },
...
process.stdout.write(
  `${[
    `PR07_CONTRACT_REVISION_SHA256=${sha256(revisionContent)}`,
...
function assertExactLaneHead() {
  ...
  if (head !== pr07LaneAnchor) {
    throw new Error(
      `PR-07 generator requires exact lane-anchor HEAD ${pr07LaneAnchor}; actual ${head}.`,
    )
  }
  execFileSync(
    "git",
    ["merge-base", "--is-ancestor", pr07ContractBase, pr07LaneAnchor],
    {
      cwd: repositoryRoot,
      stdio: "ignore",
    },
...
function assertStagedCandidate() {
  try {
    ...
    throw new Error(
      "PR-07 generator requires every tracked candidate change to be staged.",
    )
  }
  const untracked = gitOutput(["ls-files", "--others", "--exclude-standard"])
  if (untracked.length > 0) {
    throw new Error(
      `PR-07 generator rejects untracked candidate paths: ${untracked
        .split("\n")
        .join(", ")}`,
    )
  }
  ...
  if (!hasStagedChanges) {
    throw new Error("PR-07 generator requires a staged candidate delta.")
  }
}
