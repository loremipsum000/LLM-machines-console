...
if (
  ...
  throw new Error(
    "Pass exactly --print-operation-policy or --write for the PR-04 contract.",
  )
}
...
const baseEvidenceErrors = verifyPr04BaseEvidence(repositoryRoot)
if (baseEvidenceErrors.length > 0) {
  throw new Error(
    `PR-04 retained prior evidence changed: ${baseEvidenceErrors.join("; ")}`,
  )
}

const baseAllowlist = readJsonFromCommit(pr04ContractBase, allowlistPath)
const baseRoutes = readJsonFromCommit(pr04ContractBase, routeBaselinePath)
...
const decisionErrors = verifyPr04DecisionDocument(decision, {
  requireReady: true,
})
if (decisionErrors.length > 0) {
  throw new Error(
    `PR-04 decision evidence is not ready: ${decisionErrors.join("; ")}`,
  )
}
if (
  ...
  throw new Error(
    "PR-04 reviewed operation policy differs from staged candidate.",
  )
}

const evidenceFiles = pr04RevisionEvidencePaths.map((path) => ({
  path,
...
const revision = buildContractRevisionDocument({
  ...
  baseCommit: pr04ContractBase,
  baseTree: gitOutput(["rev-parse", `${pr04ContractBase}^{tree}`]),
  baseAllowlist,
  currentAllowlist,
  baseRoutes,
...
const candidateErrors = verifyPr04CandidateContract({
  root: repositoryRoot,
  baseAllowlist,
  currentAllowlist,
  baseRoutes,
...
if (candidateErrors.length > 0) {
  throw new Error(
    `PR-04 candidate violates the reviewed contract: ${candidateErrors.join("; ")}`,
  )
}
...
writeJsonTransaction([
  { path: pr04ContractRevisionPath, content: revisionContent },
...
process.stdout.write(
  `${[
    `PR04_CONTRACT_REVISION_SHA256=${sha256(revisionContent)}`,
...
function assertExactLaneHead() {
  ...
  if (head !== pr04LaneAnchor) {
    throw new Error(
      `PR-04 generator requires exact lane-anchor HEAD ${pr04LaneAnchor}; actual ${head}.`,
    )
  }
  execFileSync(
    "git",
    ["merge-base", "--is-ancestor", pr04ContractBase, pr04LaneAnchor],
    {
      cwd: repositoryRoot,
      stdio: "ignore",
    },
...
function assertStagedCandidate() {
  try {
    ...
    throw new Error(
      "PR-04 generator requires every tracked candidate change to be staged.",
    )
  }
  const untracked = gitOutput(["ls-files", "--others", "--exclude-standard"])
  if (untracked.length > 0) {
    throw new Error(
      `PR-04 generator rejects untracked candidate paths: ${untracked
        .split("\n")
        .join(", ")}`,
    )
  }
  ...
  if (!hasStagedChanges) {
    throw new Error("PR-04 generator requires a staged candidate delta.")
  }
}
