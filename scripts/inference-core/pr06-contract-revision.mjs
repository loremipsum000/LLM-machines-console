...
if (
  ...
  throw new Error(
    "Pass exactly --print-operation-policy or --write for the PR-06 contract.",
  )
}
...
const baseEvidenceErrors = verifyPr06BaseEvidence(repositoryRoot)
if (baseEvidenceErrors.length > 0) {
  throw new Error(
    `PR-06 retained prior evidence changed: ${baseEvidenceErrors.join("; ")}`,
  )
}

const baseAllowlist = readJsonFromCommit(pr06ContractBase, allowlistPath)
const baseRoutes = readJsonFromCommit(pr06ContractBase, routeBaselinePath)
...
const boundaryErrors = verifyPr06OperationBoundary(operationPolicy)
if (boundaryErrors.length > 0) {
  throw new Error(
    `PR-06 candidate leaves its approved package boundary: ${boundaryErrors.join("; ")}`,
  )
}
const candidateErrors = verifyPr06CandidateContract({
  root: repositoryRoot,
  baseAllowlist,
  currentAllowlist,
  baseRoutes,
...
if (candidateErrors.length > 0) {
  throw new Error(
    `PR-06 candidate violates the reviewed contract: ${candidateErrors.join("; ")}`,
  )
}
...
const decisionErrors = verifyPr06DecisionDocument(decision, {
  requireReady: true,
})
if (decisionErrors.length > 0) {
  throw new Error(
    `PR-06 decision evidence is not ready: ${decisionErrors.join("; ")}`,
  )
}
if (
  ...
  throw new Error(
    "PR-06 reviewed operation policy differs from staged candidate.",
  )
}

const evidenceFiles = pr06RevisionEvidencePaths.map((path) => ({
  path,
...
const revision = buildContractRevisionDocument({
  ...
  baseTree: gitOutput(["rev-parse", `${pr06ContractBase}^{tree}`]),
  baseAllowlist,
  currentAllowlist,
  baseRoutes,
  currentRoutes: currentRoutesBeforeRevision,
...
writeJsonTransaction([
  { path: pr06ContractRevisionPath, content: revisionContent },
...
process.stdout.write(
  `${[
    `PR06_CONTRACT_REVISION_SHA256=${sha256(revisionContent)}`,
...
function assertExactLaneHead() {
  ...
  if (head !== pr06LaneAnchor) {
    throw new Error(
      `PR-06 generator requires exact lane-anchor HEAD ${pr06LaneAnchor}; actual ${head}.`,
    )
  }
  execFileSync(
    "git",
    ["merge-base", "--is-ancestor", pr06ContractBase, pr06LaneAnchor],
    {
      cwd: repositoryRoot,
      stdio: "ignore",
    },
...
function assertStagedCandidate() {
  try {
    ...
    throw new Error(
      "PR-06 generator requires every tracked candidate change to be staged.",
    )
  }
  const untracked = gitOutput(["ls-files", "--others", "--exclude-standard"])
  if (untracked.length > 0) {
    throw new Error(
      `PR-06 generator rejects untracked candidate paths: ${untracked
        .split("\n")
        .join(", ")}`,
    )
  }
  ...
  if (!hasStagedChanges) {
    throw new Error("PR-06 generator requires a staged candidate delta.")
  }
}
