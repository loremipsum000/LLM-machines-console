...
if (
  ...
  throw new Error(
    "Pass exactly --print-operation-policy or --write for the PR-05 contract.",
  )
}
...
const baseEvidenceErrors = verifyPr05BaseEvidence(repositoryRoot)
if (baseEvidenceErrors.length > 0) {
  throw new Error(
    `PR-05 retained prior evidence changed: ${baseEvidenceErrors.join("; ")}`,
  )
}

const baseAllowlist = readJsonFromCommit(pr05ContractBase, allowlistPath)
const baseRoutes = readJsonFromCommit(pr05ContractBase, routeBaselinePath)
...
const boundaryErrors = verifyPr05OperationBoundary(operationPolicy)
if (boundaryErrors.length > 0) {
  throw new Error(
    `PR-05 candidate leaves its approved package boundary: ${boundaryErrors.join("; ")}`,
  )
}
const candidateErrors = verifyPr05CandidateContract({
  root: repositoryRoot,
  baseAllowlist,
  currentAllowlist,
  baseRoutes,
...
if (candidateErrors.length > 0) {
  throw new Error(
    `PR-05 candidate violates the reviewed contract: ${candidateErrors.join("; ")}`,
  )
}
...
const decisionErrors = verifyPr05DecisionDocument(decision, {
  requireReady: true,
})
if (decisionErrors.length > 0) {
  throw new Error(
    `PR-05 decision evidence is not ready: ${decisionErrors.join("; ")}`,
  )
}
if (
  ...
  throw new Error(
    "PR-05 reviewed operation policy differs from staged candidate.",
  )
}

const evidenceFiles = pr05RevisionEvidencePaths.map((path) => ({
  path,
...
const revision = buildContractRevisionDocument({
  ...
  baseTree: gitOutput(["rev-parse", `${pr05ContractBase}^{tree}`]),
  baseAllowlist,
  currentAllowlist,
  baseRoutes,
  currentRoutes: currentRoutesBeforeRevision,
...
writeJsonTransaction([
  { path: pr05ContractRevisionPath, content: revisionContent },
...
process.stdout.write(
  `${[
    `PR05_CONTRACT_REVISION_SHA256=${sha256(revisionContent)}`,
...
function assertExactLaneHead() {
  ...
  if (head !== pr05LaneAnchor) {
    throw new Error(
      `PR-05 generator requires exact lane-anchor HEAD ${pr05LaneAnchor}; actual ${head}.`,
    )
  }
  execFileSync(
    "git",
    ["merge-base", "--is-ancestor", pr05ContractBase, pr05LaneAnchor],
    {
      cwd: repositoryRoot,
      stdio: "ignore",
    },
...
function assertStagedCandidate() {
  try {
    ...
    throw new Error(
      "PR-05 generator requires every tracked candidate change to be staged.",
    )
  }
  const untracked = gitOutput(["ls-files", "--others", "--exclude-standard"])
  if (untracked.length > 0) {
    throw new Error(
      `PR-05 generator rejects untracked candidate paths: ${untracked
        .split("\n")
        .join(", ")}`,
    )
  }
  ...
  if (!hasStagedChanges) {
    throw new Error("PR-05 generator requires a staged candidate delta.")
  }
}
