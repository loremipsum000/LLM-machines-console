import "server-only"

import {
  adminConnectedAppsResponseSchema,
  adminConnectedAppDetailSchema,
  adminConnectorRegistryResponseSchema,
  adminHardwareResponseSchema,
  adminInferenceDashboardSchema,
  adminMcpServerDetailSchema,
  adminSettingsResponseSchema,
  adminTeamBreakGlassSchema,
  adminTeamGroupDetailSchema,
  adminTeamMemberDetailSchema,
  adminTeamOverviewResponseSchema,
  adminTeamScimStatusSchema,
  knowledgeArchiveSourceListResponseSchema,
  knowledgeCorpusDetailResponseSchema,
  knowledgeCorpusListResponseSchema,
  knowledgeQueryResultSchema,
  type AdminMcpServerDetail,
  type AdminConnectedAppDetail,
} from "@llm-machines/contracts"
import { getBffRequest } from "@/lib/bff/server-request"

export class ConsoleBffAuthExpiredError extends Error {
  constructor(path: string) {
    super(`Console BFF authentication expired for ${path}.`)
    this.name = "ConsoleBffAuthExpiredError"
  }
}

export function isConsoleBffAuthExpiredError(
  error: unknown,
): error is ConsoleBffAuthExpiredError {
  return error instanceof ConsoleBffAuthExpiredError
}

export async function getAdminHardware(
  filters: {
    range?: string
    step?: string
  } = {},
) {
  const params = new URLSearchParams()
  if (filters.range) {
    params.set("range", filters.range)
  }
  if (filters.step) {
    params.set("step", filters.step)
  }
  const queryString = params.toString()
  return getAdminData(
    `/api/admin/hardware${queryString ? `?${queryString}` : ""}`,
    adminHardwareResponseSchema,
  )
}

export async function getAdminInference(
  filters: {
    range?: string
  } = {},
) {
  const params = new URLSearchParams()
  if (filters.range) {
    params.set("range", filters.range)
  }
  const queryString = params.toString()
  return getAdminData(
    `/api/admin/inference${queryString ? `?${queryString}` : ""}`,
    adminInferenceDashboardSchema,
  )
}

export async function getAdminConnectorRegistry(
  filters: {
    query?: string
  } = {},
) {
  const params = new URLSearchParams()
  if (filters.query) {
    params.set("q", filters.query)
  }
  const queryString = params.toString()

  return getAdminData(
    `/api/admin/agents/registry${queryString ? `?${queryString}` : ""}`,
    adminConnectorRegistryResponseSchema,
  )
}

export async function getAdminConnectedApps() {
  return getAdminData(
    "/api/admin/applications/connected-apps",
    adminConnectedAppsResponseSchema,
  )
}

export async function getAdminConnectedAppDetail(
  appId: string,
): Promise<AdminConnectedAppDetail | null> {
  return getNullableAdminData(
    `/api/admin/applications/connected-apps/${encodeURIComponent(appId)}`,
    adminConnectedAppDetailSchema,
  )
}

export async function getAdminMcpServerDetail(
  connectorId: string,
): Promise<AdminMcpServerDetail | null> {
  return getNullableAdminData(
    `/api/admin/mcp-servers/${encodeURIComponent(connectorId)}`,
    adminMcpServerDetailSchema,
  )
}

export async function getAdminSettings() {
  return getAdminData(
    "/api/admin/settings",
    adminSettingsResponseSchema,
  )
}

export async function getAdminTeamOverview() {
  return getAdminData(
    "/api/admin/team",
    adminTeamOverviewResponseSchema,
  )
}

export async function getAdminTeamScimStatus() {
  return getAdminData(
    "/api/admin/team/scim",
    adminTeamScimStatusSchema,
  )
}

export async function getAdminTeamBreakGlass() {
  return getAdminData(
    "/api/admin/team/break-glass",
    adminTeamBreakGlassSchema,
  )
}

export async function getAdminTeamMemberDetail(memberId: string) {
  return getNullableAdminData(
    `/api/admin/team/members/${encodeURIComponent(memberId)}`,
    adminTeamMemberDetailSchema,
  )
}

export async function getAdminTeamGroupDetail(groupId: string) {
  return getNullableAdminData(
    `/api/admin/team/groups/${encodeURIComponent(groupId)}`,
    adminTeamGroupDetailSchema,
  )
}

export async function getAdminKnowledgeCorpora() {
  return getAdminData(
    "/api/admin/knowledge/corpora",
    knowledgeCorpusListResponseSchema,
  )
}

export async function getAdminKnowledgeArchivedSources() {
  return getAdminData(
    "/api/admin/knowledge/archive/sources",
    knowledgeArchiveSourceListResponseSchema,
  )
}

export async function getAdminKnowledgeCorpusDetail(corpusId: string) {
  const detail = await getAdminData(
    `/api/admin/knowledge/corpora/${encodeURIComponent(corpusId)}`,
    knowledgeCorpusDetailResponseSchema,
  )
  return detail ?? null
}

export async function getAdminKnowledgeRetrievalTest(
  corpusId: string,
  query: string,
) {
  const trimmedQuery = query.trim()
  if (!trimmedQuery) {
    return null
  }
  const bffRequest = await getBffRequest()
  const path = `/api/admin/knowledge/corpora/${encodeURIComponent(
    corpusId,
  )}/retrieval-test`

  if (!bffRequest) {
    throw new Error(
      `Console BFF is not available for ${path}; fixture mode is disabled.`,
    )
  }

  const response = await fetch(`${bffRequest.baseUrl}${path}`, {
    body: JSON.stringify({
      corpusIds: [corpusId],
      query: trimmedQuery,
      topK: 5,
    }),
    cache: "no-store",
    headers: {
      ...(bffRequest.headers as Record<string, string>),
      "Content-Type": "application/json",
    },
    method: "POST",
  })

  if (!response.ok) {
    if (response.status === 401) {
      throw new ConsoleBffAuthExpiredError(path)
    }
    throw new Error(`Console BFF returned HTTP ${response.status} for ${path}.`)
  }

  return knowledgeQueryResultSchema.parse(await response.json())
}

async function getAdminData<T>(
  path: string,
  schema: { parse: (value: unknown) => T },
) {
  const bffRequest = await getBffRequest()
  if (!bffRequest) {
    throw new Error(
      `Console BFF is not available for ${path}; fixture mode is disabled.`,
    )
  }

  try {
    const response = await fetch(`${bffRequest.baseUrl}${path}`, {
      cache: "no-store",
      headers: bffRequest.headers,
    })

    if (!response.ok) {
      if (response.status === 401) {
        throw new ConsoleBffAuthExpiredError(path)
      }
      throw new Error(
        `Console BFF returned HTTP ${response.status} for ${path}.`,
      )
    }

    return schema.parse(await response.json())
  } catch (error) {
    if (error instanceof Error) {
      throw error
    }
    throw new Error(`Console BFF request failed for ${path}.`)
  }
}

async function getNullableAdminData<T>(
  path: string,
  schema: { parse: (value: unknown) => T },
) {
  const bffRequest = await getBffRequest()
  if (!bffRequest) {
    throw new Error(
      `Console BFF is not available for ${path}; fixture mode is disabled.`,
    )
  }

  try {
    const response = await fetch(`${bffRequest.baseUrl}${path}`, {
      cache: "no-store",
      headers: bffRequest.headers,
    })

    if (response.status === 401) {
      throw new ConsoleBffAuthExpiredError(path)
    }
    if (response.status === 403 || response.status === 404) {
      return null
    }
    if (!response.ok) {
      throw new Error(
        `Console BFF returned HTTP ${response.status} for ${path}.`,
      )
    }

    return schema.parse(await response.json())
  } catch (error) {
    if (error instanceof Error) {
      throw error
    }
    throw new Error(`Console BFF request failed for ${path}.`)
  }
}
