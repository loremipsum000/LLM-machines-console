...
`NO_RELEASE_ROLLBACK`. Its verifier requires credential-free commissioning
evidence that no prior Product release exists. It remains runtime-unqualified,
inactive, and blocked on `Q0_PREINSTALL_BACKUP_AND_CLEAN_RESTORE`. Any later
release continues to require a different, completely signed and independently
verified predecessor release; the first-release descriptor cannot be reused.
