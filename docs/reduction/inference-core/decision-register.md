...
| F0-N8 | Retained native-access pre-Genesis closure | `f0-n8-native-access-closure.json` | Governance-only candidate binds protected PRs 90 through 107, including every retained-native successor and the passing F0-N7 VM117 evidence, while preserving the intervening F0-F2R merge as unrelated protected history. Console remains the primary complete customer surface; Grafana, LiteLLM, and scoped Keycloak administration retain their admitted roles and native sessions; Portainer remains `DEFERRED_UPSTREAM_SECURITY`. Product acceptance and runtime qualification remain false, activation remains inactive pending separate VM103 deployment approval, Q0 is not started, Genesis is unpublished, and Product main is unchanged. |
| F0-N4R1 | Prospective Portainer CE source-security reassessment | `f0-n4r1-portainer-security-admission.json`, `infra/portainer/ce-downstream/source-package.json`, and `infra/portainer/ce-downstream/security-finding-matrix.json` | Founder-authorized, same-version re-evaluation selects a source-built Portainer CE `2.39.6-llmm.1` downstream from exact upstream commit `723d1a2268f0fefe70d57f5981ce15d5d1ffc679`. Fresh evidence is separate from the missing historical raw artifacts and does not rewrite or retroactively invalidate F0-N4. Two independent assemblies produce the same exact OCI archive and canonical inventory; shipped frontend evidence is separately bound; and license custody reviews all 581 expected components with no gaps, unknown expressions, prohibited material, Business Edition, trial, or commercial-license material. Raw Trivy and govulncheck findings are preserved rather than reported as zero; the complete matrix records zero unresolved reachable Critical or High findings under the source-only profile and retains exact activation preconditions. A separate exact Dockerfile configuration scan records DS-0002 HIGH effective root and DS-0026 LOW missing image healthcheck as activation-blocking F0-N4R2 privilege and health-contract requirements; it is a post-build static review, not a Final5 aggregate input or a claim that either finding is resolved. Portainer is required for the prospective retained OSS boundary but is not yet Core-admitted, locked, started, routed, linked, packaged, deployed, or advertised. Status is `SOURCE_SECURITY_CHARACTERIZED_NOT_CORE_ADMITTED`; accepted and runtimeQualified remain false; contract activation remains inactive; Q0 is not started; Genesis is unpublished. |
